---
title: This site
tagline: A static portfolio with typed content and a script budget the build enforces.
kind: also-built
status: personal
period: { start: '2026-09', end: null }
stack: [astro, typescript, vitest]
evidence:
  - kind: repo
    label: thomazzi98/thomazzi98.github.io
    url: https://github.com/thomazzi98/thomazzi98.github.io
---

Roles, projects and education come from typed content collections. A technology that no role or
project references fails the build, and so does a reference to a technology that is not in the
registry. Every page stays under the script budget
the build enforces. The colophon describes the rest.
